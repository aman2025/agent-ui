...
/**
 * Normalize form data keys by stripping common prefixes
 * Converts keys like "formValues.instanceName" to "instanceName"
 * @param {Object} formData - Raw form data with potentially prefixed keys
 * @returns {Object} Normalized form data with clean parameter names
 */
function normalizeFormData(formData) {
  ...
  for (const [key, value] of Object.entries(formData)) {
    // Strip common prefixes: formValues., form., values.
    const cleanKey = key
      .replace(/^formValues\./, '')
      .replace(/^form\./, '')
      .replace(/^values\./, '');
...
class ReActAgent {
  ...
  async process(query, context = {}) {
    console.log('[DEBUG] ReActAgent.process - Starting with query:', query);
    this.state = AgentState.REASONING;
    
    // Step 1: REASON - Analyze query and determine intent
    console.log('[DEBUG] ReActAgent.process - Starting REASON phase...');
    const reasonStart = Date.now();
    const reasoning = await this.reason(query, context);
    console.log('[DEBUG] ReActAgent.process - REASON completed in', Date.now() - reasonStart, 'ms');
    
    // Step 2: ACT - Generate appropriate UI response
    this.state = AgentState.ACTING;
    console.log('[DEBUG] ReActAgent.process - Starting ACT phase...');
    const actStart = Date.now();
    const action = await this.act(reasoning, context);
    console.log('[DEBUG] ReActAgent.process - ACT completed in', Date.now() - actStart, 'ms');
  ...
  async processAction(actionId, formData, context = {}) {
    ...
    // Normalize form data keys (strip prefixes like "formValues.")
    const normalizedFormData = normalizeFormData(formData);
    let currentContext = { ...context, formData: normalizedFormData };
    
    while (retryCount < this.maxRetries) {
      // Step 1: ACT - Execute the tool
      this.state = AgentState.ACTING;
      const toolResult = await this.executeTool(actionId, normalizedFormData);
  ...
  async reason(query, context) {
    console.log('[DEBUG] ReActAgent.reason - Composing prompts...');
    ...
    console.log('[DEBUG] ReActAgent.reason - Calling LLM...');
    ...
    console.log('[DEBUG] ReActAgent.reason - LLM responded in', Date.now() - llmStart, 'ms');
  ...
  async act(reasoning, context) {
    ...
    console.log('[DEBUG] ReActAgent.act - Calling LLM with:');
    console.log('[DEBUG] - Phase: acting');
    console.log('[DEBUG] - Reasoning:', JSON.stringify(reasoning, null, 2));
    ...
    console.log('[DEBUG] ReActAgent.act - LLM response received:');
    ...
    console.log('[DEBUG] ReActAgent.act - JSON string for parsing:', jsonString);
    
    const parseResult = parse(jsonString);
    if (!parseResult.success) {
      console.log('[DEBUG] ReActAgent.act - Parse failed:');
      console.log('[DEBUG] - Error code:', parseResult.error.code);
      console.log('[DEBUG] - Error message:', parseResult.error.message);
      console.log('[DEBUG] - Error details:', JSON.stringify(parseResult.error.details, null, 2));
      throw new Error(`Invalid UI structure: ${parseResult.error.message}`);
    }
    
    console.log('[DEBUG] ReActAgent.act - Parse successful');
  ...
  async generateResultUI(result, context) {
    ...
    return {
      type: 'result',
      ui: parseResult.data,
      toolResult: result, // Include tool result for dataModel binding
