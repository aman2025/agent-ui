...
class MistralClient {
  ...
  async chat({ systemPrompt, userPrompt, responseFormat = 'json' }) {
    let lastError = null;
    console.log('[DEBUG] MistralClient.chat - Starting request');
    console.log('[DEBUG] MistralClient.chat - Model:', this.model);
    console.log('[DEBUG] MistralClient.chat - System prompt length:', systemPrompt?.length);
    console.log('[DEBUG] MistralClient.chat - User prompt length:', userPrompt?.length);

    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        console.log('[DEBUG] MistralClient.chat - Attempt', attempt + 1, 'of', this.maxRetries + 1);
        const apiStart = Date.now();
        
        // Add timeout wrapper (60 seconds)
        const timeoutMs = 60000;
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(() => reject(new Error(`Mistral API timeout after ${timeoutMs}ms`)), timeoutMs);
        });
        
        const apiPromise = this.client.chat.complete({
          model: this.model,
          messages: [
        ...
        console.log('[DEBUG] MistralClient.chat - Waiting for Mistral response (timeout: 60s)...');
        const response = await Promise.race([apiPromise, timeoutPromise]);
        
        console.log('[DEBUG] MistralClient.chat - API call completed in', Date.now() - apiStart, 'ms');

        const content = response.choices?.[0]?.message?.content;

        // Debug: Log raw Mistral response
        console.log('[DEBUG] Mistral LLM Response:');
        console.log('[DEBUG] - Model:', this.model);
        console.log('[DEBUG] - Response format:', responseFormat);
